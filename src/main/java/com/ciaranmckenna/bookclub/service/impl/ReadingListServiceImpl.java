...
public class ReadingListServiceImpl implements ReadingListService {
    ...
    public ReadingListDto getReadingListById(Long id) {
        ReadingList readingList = readingListRepository.findWithBooksById(id)
                .orElseThrow(() -> new EntityNotFoundException("Reading list not found with id: " + id));
    ...
    public Page<ReadingListDto> searchReadingListsByName(String name, Pageable pageable) {
        return readingListRepository.findByNameContainingIgnoreCase(name, pageable)
                .map(this::convertToDto);
    }
     ...
     */
    @Override
    public Page<ReadingListDto> searchReadingListsByNameAndUserId(String name, Long userId, Pageable pageable) {
        return readingListRepository.findByNameContainingIgnoreCaseAndUserId(name, userId, pageable)
                .map(this::convertToDto);
    }
    ...
    public ReadingListDto addBookToReadingList(Long readingListId, Long bookId) {
        ReadingList readingList = readingListRepository.findById(readingListId)
                .orElseThrow(() -> new EntityNotFoundException("Reading list not found with id: " + readingListId));
    ...
    private ReadingListDto convertToDto(ReadingList readingList) {
        ...
        if (readingList.getBooks() != null) {
            bookDtos = readingList.getBooks().stream()
